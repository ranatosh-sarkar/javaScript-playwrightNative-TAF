...
test.describe('Home Page: Verifying Home page', () => {

  test('Verify Core UI elements', async ({ page, homeData }) => {
    ...
    test('Verify Header and footer navigation sections', async ({ page, homeData }) => {
    const home = new HomePage(page);
    ...
    // 4) Verify header navigation links (texts from Excel)
    ...
    // 5) Verify footer navigation links (texts from Excel)
    ...
    test('Date fields are auto-populated with today and tomorrow correctly', async ({ page, homeData }) => {
    const home = new HomePage(page);

    // 1) Navigate -> already done; assert page loaded
    await home.expectWelcomeTextVisibility();

    // 2) Verify title + welcome text
    await home.verifyTitle(homeData.expected_title);
    await home.verifyWelcomeText(homeData.welcome_text);

    // 3) Default Check-In = today (dd/MM/yyyy)
    await home.verifyDefaultCheckinDate('today');

    // 4) Default Check-Out = tomorrow (dd/MM/yyyy)
    await home.verifyDefaultCheckoutDate('tomorrow');

    Logger.info(`Dates verified: check-in= (today), check-out= (tomorrow)`);
  });
